...
#define BLOCKING 1          // Software blocking algorithm
#define BLOCKING_FACTOR 8  // Blocking Factor
...
*/
void get_index_size(int argc, char *argv[], int *dim_l, int *dim_m, int *dim_n, int* block_size) {
  if (argc != 2 && argc != 3) {
    printf("usage:  mm_mult_serial [l_dimension] <block_size> \n");
    exit(1);
  }
  else {
	  if (argc == 2) {
...
int main(int argc, char *argv[])
{
  ...
  get matrix sizes
  */
  get_index_size(argc, argv, &dim_l, &dim_m, &dim_n, &block_size);

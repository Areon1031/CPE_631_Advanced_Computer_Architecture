...
#define BLOCKING 0          // Software blocking algorithm
#define BLOCKING_FACTOR 64  // Blocking Factor
...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  for (i = 0; i<dim_l; i++) {
    ...
    for (j = 0; j<dim_n; j++) {
      dot_prod = 0.0;
      #pragma omp parallel for shared(dot_prod)
      for (k = 0; k<dim_m; k++) {
...
#else
  int BLOCK = BLOCKING_FACTOR;
  float r = 0;
  for (int jj = 0; jj < dim_m; jj = jj + BLOCK)
  {
    for (int kk = 0; kk < dim_m; kk = kk + BLOCK)
    {
      for (int i = 0; i < dim_m; i = i + 1)
      {
        for (int j = jj; j < MIN(kk + BLOCK, dim_m); j = j + 1)
        {
          r = 0;
          for (int k = kk; k < MIN(kk + BLOCK, dim_m); k = k + 1)
          {
            r = r + A(i, k)*B(k, j);
          }
          C(i, j) = C(i, j) + r;
